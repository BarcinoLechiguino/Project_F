...
void UI_Button::CheckInput()
{
	...
	if (is_visible)																				//If the Button element is visible.
	{
		...
		if ((IsHovered() || is_drag_target) && App->input->GetMouseButtonDown(SDL_BUTTON_LEFT) == KEY_REPEAT)	//If the mouse is on the button and the left mouse button is pressed continuously.
		{
		...
		if ((IsHovered() || is_drag_target) && App->input->GetMouseButtonDown(SDL_BUTTON_LEFT) == KEY_UP)			//If the mouse is on the button and the left mouse button is released.
		{
