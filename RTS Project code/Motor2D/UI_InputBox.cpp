...
void UI_InputBox::CheckInput()
{
	if (is_visible)
	{
		...
		if (is_interactible)																		//If the Text element is interactible.
		{
			...
			if (is_draggable)
			{
				if ((IsHovered() || is_drag_target) && App->input->GetMouseButtonDown(SDL_BUTTON_LEFT) == KEY_REPEAT)		//If the mouse is on the text and the left mouse button is being pressed.
				{
			...
			if ((IsHovered() || is_drag_target) && App->input->GetMouseButtonDown(SDL_BUTTON_LEFT) == KEY_UP)	//If the mouse is on the text and the left mouse button is released.
			{
...
void UI_InputBox::RefreshInputText()														// --------------------------------------------------------------------------
{
	...
	if (previous_length != TextLength())													//If there has been a change in the string (Addition, deletion). Takes into account the length of the string in characters and cursorIndex.
	{
		RefreshCursorPos();
