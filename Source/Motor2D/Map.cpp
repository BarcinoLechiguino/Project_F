...
bool Map::LoadObjectLayers(pugi::xml_node& node, ObjectGroup * objectgroup)
{
	...
	for (pugi::xml_node object_iterator = node.child("object"); object_iterator; object_iterator = object_iterator.next_sibling("object"))
	{
		...
		else if (object_type == "enemy_barracks")
		{
			...
			App->enemy_AI_manager->enemy_barracks = (EnemyBarracks*)App->entity_manager->CreateEntity(ENTITY_TYPE::ENEMY_BARRACKS, tile_coords.x, tile_coords.y);
		}
		else if (object_type == "enemy_hall")
		{
			...
			App->enemy_AI_manager->enemy_townhall = (EnemyTownHall*)App->entity_manager->CreateEntity(ENTITY_TYPE::ENEMY_TOWNHALL, tile_coords.x, tile_coords.y);
		...
		else if (object_type == "tutorial_boulder")
		{
			//objectgroup->object[index].type = HEAVY;
			tutorial_boulders.push_back(App->entity_manager->CreateEntity(ENTITY_TYPE::BOULDER, tile_coords.x, tile_coords.y));
		}
