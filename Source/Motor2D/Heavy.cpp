...
Heavy::Heavy(int x, int y, ENTITY_TYPE type, int level) : AllyUnit(x, y, type, level)  //Constructor. Called at the first frame.
{
	LOG("x %d and y %d", x, y);
	InitEntity();
};
...
void Heavy::InitUnitSpriteSections()
{
	...
	//pathing_down_left_section = { 0, 0, 58, 47 };
		...
		//	 --- LOADING FROM XML ---
	pugi::xml_node sections = App->entities_file.child("entities").child("units").child("allies").child("heavy").child("sprite_sections");
